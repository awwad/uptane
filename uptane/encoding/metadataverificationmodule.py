...
Positive.subtypeSpec = constraint.ValueRangeConstraint(0, MAX) # TODO: <~> Had to change this from 1-MAX to 0-MAX because there is a conversion call in TUF for a signature status check before version has been incremented to 1, while version is 0 (for whatever reason). Clear up later.
